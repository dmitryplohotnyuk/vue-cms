var _expandTransition = require('vuetify/es5/components/transitions/expand-transition')
            ...
            for (var index = 0, len = this.filteredItems.length; index < len; ++index) {
                ...
                rows.push(
                    this.hasTag(row, 'td')
                        ? this.genTR(row, {
                              key: this.itemKey
                                  ? (0, _helpers.getObjectValueByPath)(props.item, this.itemKey)
                                  : index,
                ...
                if (this.$scopedSlots.expand) {
                    ...
                    // rows.push(expandRow)
                    if (this.isExpanded(props.item) || props.item.expanded) {
                        rows.push(this.$scopedSlots.expand(props))
                    }
                }
            }
            return rows
